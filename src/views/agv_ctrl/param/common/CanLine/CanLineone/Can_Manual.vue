<template>
    <div>
      <el-table :data="CANMANUAL" style="width: 100%">
...
import { CANMANUAL } from '@/views/agv_ctrl/param/common/commondata.js'


const addNewRow = () => {
    CANMANUAL.value.push({
    ID: '',
    canID: '',
  });
};
...
const deleteRow = (index) => {
    CANMANUAL.value.splice(index, 1);
};
...
jsondata.value.can1 = {
  ...jsondata.value.can1,
  can_manual: computed(() => {
    return CANMANUAL.value.map((row) => [
